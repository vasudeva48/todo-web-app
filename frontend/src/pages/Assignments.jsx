    ...
    <div className="space-y-6">
      ...
      <GlassCard className="p-6" hover={false}>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4.5 py-2.5 text-xs font-semibold rounded-xl text-white bg-blue-600 hover:bg-blue-750 shadow-md shadow-blue-500/10 hover:shadow-blue-500/20 active:scale-98 transition-all"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
          ...
          <div className="relative md:col-span-6">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/40 text-xs text-slate-850 dark:text-zinc-100 focus:border-blue-500 outline-none"
            />
          </div>
          ...
          <div className="relative md:col-span-3 flex gap-2">
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/40 text-xs text-slate-850 dark:text-zinc-100 focus:border-blue-500 outline-none"
            >
          ...
          <div className="relative md:col-span-3">
            <button
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/40 text-xs font-semibold hover:bg-slate-50 dark:hover:bg-zinc-900 transition-colors text-slate-700 dark:text-zinc-350"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="flex flex-col gap-1 items-start">
            ...
            <select
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/40 text-xs focus:border-blue-500 outline-none"
            >
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="glass-panel rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 border border-slate-200/50 dark:border-zinc-800/50">
            
            {/* Desktop Table View */}
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200/40 dark:border-zinc-850/60 bg-slate-50/50 dark:bg-zinc-950/30">
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider w-10">Select</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Assignment Title</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Course Subject</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Priority</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Due Date</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/40">
                  {paginatedTasks.map((task) => (
                    <tr 
                      key={task._id} 
                      className={`hover:bg-slate-50/40 dark:hover:bg-zinc-950/20 transition-colors ${
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <button
                          ...
                          className={`w-5 h-5 flex items-center justify-center rounded-lg border transition-all ${
                            task.completed
                              ? 'bg-emerald-500 border-emerald-500 text-white cursor-default'
                              : 'border-slate-350 hover:border-blue-500 dark:border-zinc-700 bg-transparent text-transparent hover:text-blue-500'
                          }`}
                        >
                      ...
                      <td className="py-3 px-4 min-w-[200px]">
                        <span className={`text-xs font-bold ${task.completed ? 'line-through text-slate-400' : 'text-slate-700 dark:text-zinc-100'}`}>
                          {task.title}
                        </span>
                      </td>
                      ...
                      {/* Priority */}
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${getPriorityBadge(task.priority)}`}>
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
                            title="View details"
                          >
                          ...
                          {!task.completed && (
                            <button
                              ...
                              className="p-1.5 text-slate-400 hover:text-slate-650 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
                              title="Edit"
                            >
            ...
            <div className="block md:hidden divide-y divide-slate-100 dark:divide-zinc-850">
              {paginatedTasks.map((task) => (
                <div key={task._id} className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold ${task.completed ? 'line-through text-slate-450' : 'text-slate-800 dark:text-zinc-150'}`}>
                      {task.title}
                    </span>
                    <button
                      onClick={() => !task.completed && handleComplete(task)}
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <span className={`inline-flex items-center gap-0.5 px-2 py-0.5 rounded text-[10px] ${getPriorityBadge(task.priority)}`}>
                      {task.priority}
                    </span>
                    ...
                    <span className="text-slate-550 flex items-center gap-1 font-medium">
                      <Clock className="w-3.5 h-3.5 text-slate-400" />
                      <span>{task.dueDate || 'No deadline'}</span>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-2">
                    <button
                      onClick={() => { setSelectedTask(task); setIsDetailOpen(true); }}
                      className="px-2.5 py-1.5 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-[10px] font-bold rounded-lg text-slate-600 dark:text-zinc-300"
                    >
                      Details
                    </button>
                    {!task.completed && (
                    ...
                    <button
                      onClick={() => onDeleteTask(task._id)}
                      className="px-2.5 py-1.5 bg-rose-50 dark:bg-rose-950/20 text-[10px] font-bold rounded-lg text-rose-600 dark:text-rose-400"
                    >
                      Delete
                    </button>
                  </div>
